...
public final class SincImageResampler
{
	 ...
	 * <li>If {@code outputFile} is non-{@code null} and {@code outputFileType} is {@code null}: The type is set to {@code "png"} or
	 * {@code "bmp"} if the output file has that extension (case-insensitive), otherwise it is set to {@code "png"} by default.</li>
	 * <li>If {@code outputFile} is non-{@code null}, then {@code outputImage} is written to the file.</li>
	 * </ol>
	 * <p>
	 * After calling {@code run()}, it is recommend that this object should not be reused for another resampling operation. This is because
	 * various fields probably need to be cleared, such as the filter length and output file type.</p>
	 *
